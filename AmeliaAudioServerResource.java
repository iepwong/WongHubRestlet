 ...
 */
public class AmeliaAudioServerResource extends ServerResource {
	...
	public void poststore(JsonRepresentation NinjaRep) throws JSONException {
		...
		String PrevAudioStr = jedis.get("PrevAmeliaAudio");
		
		if (PrevAudioStr == null) PrevAudioStr = "0";
		jedis.set("PrevAmeliaAudio", DA);
		jedis.set("AmeliaAudioTime", parser1.print(currentdatetime));
	
		if (Float.parseFloat(DA) >= AUDIOTHRESH){
		try
		{
			System.out.println("Amelia Audio Tripped: " +DA+ " -> "+ inDate);
			// pushoverClient.PushoverClientPost("Amelia Audio Tripped -> "+DA, "AmeliaSleep");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
